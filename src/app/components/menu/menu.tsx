  ...
  useEffect(() => {
    ...
    if (storedProjects) {
      ...
      const filteredProjects = project.filter((p) =>
        p?.project?.toLowerCase().includes(searchText.toLowerCase())
      );
        ...
        >
          <MenuHandler>
            <Typography as="div" variant="small" className="font-medium">
              <ListItem
                className={`${
                  isMobileMenuOpen ? "!bg-[#ECD9F5]" : "!bg-transparent"
                }   flex items-center  justify-between gap-2 py-2  pr-4 font-medium text-sm`}
                selected={isMenuOpen || isMobileMenuOpen}
                onClick={() => setIsMobileMenuOpen((cur) => !cur)}
              >
                <div
                  className={`${
                    isMobileMenuOpen ? "text-[#7C00BE]" : ""
                ...
                <ChevronDownIcon
                  ...
                  strokeWidth={2.5}
                  className={`hidden h-3 w-3 transition-transform lg:block !self-end${
